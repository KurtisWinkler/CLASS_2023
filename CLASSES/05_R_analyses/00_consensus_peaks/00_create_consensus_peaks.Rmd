...
# CLASS CODE
setwd("/scratch/Shares/rinnclass/CLASS_2023/kurt/CLASS_2023/CLASSES/05_R_analyses/00_consensus_peaks")
...
# let's gather some data about peak numbers and save
# let's see the num_peaks for each consensus 
num_consensus_peaks <- sapply(consensus_list, length) %>% 
  as.data.frame() %>%
  rownames_to_column( var = "dbp") %>%
  dplyr::rename(number_consensus_peaks = ".")
...
peak_1 <- read_tsv("BRCA1_consensus_peaks.bed", col_names = F)
...
# FIRST a file list:
consensus_file_list <- list.files("/scratch/Shares/rinnclass/CLASS_2023/kurt/CLASS_2023/CLASSES/05_R_analyses/00_consensus_peaks", full.names = T, pattern = ".bed")
...
# input into function
export_path <- file.path(consensusFilePath, "test")
